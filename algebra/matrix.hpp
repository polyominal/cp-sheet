...
template <class T>
int mat_rank(Vec<Vec<T>> a) {
    return sweep<true>(a).first;
}

template <class T>
T mat_det(Vec<Vec<T>> a) {
    return sweep<false>(a).second;
}
