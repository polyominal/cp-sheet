...
struct LazySegtree {
    ...
    template <class A>
    LazySegtree(int n_, A a, M m_) : m(m_) {
        build(n_, a);
    }
    template <class A>
    ...
    template <class G>
    void enumerate(int l, int r, G g) {  /// start-hash
        assert(0 <= l && l <= r && r <= n);
        if (l == r) {
            return;
        }
    ...
    template <class P>
    pair<int, S> max_right(int l, P p) {  /// start-hash
        assert(0 <= l && l <= n);
        if (l == n) {
            return {n, m.e()};
        }
    ...
    template <class P>
    pair<int, S> min_left(int r, P p) {  /// start-hash
        assert(0 <= r && r <= n);
        if (r == 0) {
            return {0, m.e()};
        }
