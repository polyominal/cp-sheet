...
template <class T>
double distLP(const L<T>& l, const Point<T>& p) {
    return abs(crs(vec(l), p - l.s)) / dist(l);
}  /// end-hash

template <class T>
double distSP(const L<T>& s, const Point<T>& p) {
...
template <class T>
double distSS(const L<T>& s, const L<T>& t) {
    if (insSS(s, t)) {
        return 0;  /// start-hash
    }
...
// TODO usage
template <class T>
int crossSS(const L<T>& l, const L<T>& m, Point<T>& r) {
