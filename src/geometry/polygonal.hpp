...
template <class T>
int contains(const Vec<Point<T>>& pol, const Point<T>& p) {
    if (pol.empty()) {
        return 0;  /// start-hash
    }
    int in = -1;
...
template <class T>
Vec<Point<T>> convex_lower(const Vec<Point<T>>& pts) {
...
template <class T>
Vec<Point<T>> convex(Vec<Point<T>> pts) {
...
void diameter(const Vec<Point<T>> pol, F f) {
