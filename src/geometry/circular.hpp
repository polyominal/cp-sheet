...
inline int contains(const C& c, const P& p) { /// start-hash
...
inline int insCC(const C& a, const C& b){ /// start-hash
	D c = dist(a.c - b.c);
...
inline C incircle(const P& a, const P& b, const P& c) { /// start-hash
	D da = dist(b - c);
	D db = dist(a - c);
	D dc = dist(a - b);
	D s = da + db + dc;
...
inline C outcircle(const P& a, P b, P c) { /// start-hash
	b -= a, c -= a;
	D bb = dist2(b) / 2;
	D cc = dist2(c) / 2;
	D g = crs(b, c);
...
inline int crossCL(const C& c, const L& l, array<P, 2>& res) { /// start-hash
	D u = distLP(l, c.c);
	int t = sgn(u, c.r);
	if (t == 1) return 0;
	P v = project(l, c.c);
...
inline pair<D, D> crossCC_args(const C& a, const C& b) { /// start-hash
	P diff = b.c - a.c;
	D c = arg(diff);
...
inline int crossCC(const C& a, const C& b, array<P, 2>& res) { /// start-hash
	int t = insCC(a, b);
...
inline int tangent(const C& c, const P& p, array<P, 2>& res) { /// start-hash
	P diff = p - c.c;
	D dd = dist(diff);
	int t = sgn(c.r, dd);
	if (t == 1) return 0;
