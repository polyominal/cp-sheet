...
template <class T>
int contains(const C<T>& c, const Point<T>& p) {
...
template <class T>
int insCC(const C<T>& a, const C<T>& b) {
...
template <class T>
C<T> outcircle(const Point<T>& a, Point<T> b, Point<T> c) {
    b -= a, c -= a;  /// start-hash
    T bb = dist2(b) / 2;
    T cc = dist2(c) / 2;
    T g = crs(b, c);
...
template <class T>
pair<T, T> crossCC_args(const C<T>& a, const C<T>& b) {
