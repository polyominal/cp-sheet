...
template <class T> tuple<T, T, T> extgcd(T a, T b) {  /// start-hash
	if (b == 0) {
		return {1, 0, a};
	}
...
template <class T> Opt<T> minv(T a, T m) {	/// start-hash
	a %= m;
...
template <class T> Opt<pair<T, T>> crt(T a1, T m1, T a2, T m2) {  /// start-hash
	auto [x, y, g] = extgcd(m1, m2);
	a1 = fix(a1, m1);
	a2 = fix(a2, m2);
	if (a1 % g != a2 % g) {
		return std::nullopt;
	...
	y = fix(i128(y) * a1, m1);
	T lcm = m1 * (m2 / g);
	return pair<T, T>(fix(i128(x) * (m1 / g) + i128(y) * (m2 / g), lcm), lcm);
}  /// end-hash

template <class T>
Opt<pair<T, T>> crt(const Vec<pair<T, T>>& cons) {	/// start-hash
	auto res = Opt<pair<T, T>>({0, 1});
	for (auto [a, m] : cons) {
...
template <class T> Opt<tuple<T, T, T>> find_any(T a, T b, T c) {  /// start-hash
	assert(a && b);
