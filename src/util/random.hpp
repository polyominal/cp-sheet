...
public:
    ...
    template <class Iterator> void shuffle(Iterator first, Iterator last) {
    ...
    template <class T> std::vector<T> permutation(size_t n) {
    ...
    template <class T> std::vector<T> sample(size_t n, T lower, T upper) {
...
private:
    ...
    uint64_t next(uint64_t upper) {
        ...
        uint64_t mask = (lg == 63 ? uint64_t(-1) : (uint64_t(1) << (lg + 1)) - 1);
        while (true) {
