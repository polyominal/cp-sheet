...
template <class Node> struct Segbeats {
	int sz, h;
	Vec<Node> d;
	template <class A> Segbeats(int n, A a) {
		sz = std::bit_ceil<u32>(n);
		h = std::countr_zero<u32>(sz);
		d.resize(2 * sz);
		for (int i = 0; i < n; ++i) d[sz + i] = Node(a(i));
	...
	template <class T> void apply(int l, int r, T x) {
		if (l == r) return;
		l += sz, r += sz;
		for (int i = h; i >= 1; i--) {
			if (((l >> i) << i) != l) push(l >> i);
	...
	template <class T> void apply(int i, T x) {
