    ...
    if start >= 0:
        ...
        while end+1<len(inp) and bracketcount>0:
            ...
            if inp[end] == '(':
                ...
                bracketcount = bracketcount - 1
        if bracketcount == 0:
            return r"%s\bigo{%s}%s" % (inp[:start], inp[start+2:end], ordoescape(inp[end+1:], False))
    return inp
...
def processwithcomments(caption, instream, outstream, listingslang=None):
    ...
    for line in lines:
        ...
        if had_comment and tail == "end-hash":
            cur_hash = '\n'.join(cur_hash)
            p = subprocess.Popen(['sh', '../src/contest/%s.sh' % hash_script], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    ...
    if error:
        ...
        includelist = (include for include in includelist if include.find('contest/base.hpp') == -1)
        if includelist:
