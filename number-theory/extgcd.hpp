...
template <class T> tuple<T, T, T> extgcd(T a, T b) {
	if (b == 0) {  /// start-hash
		return {1, 0, a};
	}
...
template <class T> Opt<T> minv(T a, T m) {
	a %= m;	 /// start-hash
...
template <class T> Opt<pair<T, T>> crt(T a1, T m1, T a2, T m2) {
...
template <class T> Opt<pair<T, T>> crt(const Vec<pair<T, T>>& cons) {
...
template <class T> Opt<tuple<T, T, T>> find_any(T a, T b, T c) {
...
template <class T> T count_sols(T a, T b, T c, T xl, T xr, T yl, T yr) {
	if (c < 0) {  /// start-hash
		a = -a, b = -b, c = -c;
	}
	if (a < 0) {
